    ...
    >
      <div className="mx-auto max-w-3xl px-6 text-center">
        ...
        <h2 className="font-serif text-3xl font-bold tracking-tight text-foreground md:text-4xl text-balance">
          {"Je limite volontairement le nombre d'eleves"}
        </h2>

        <p className="mt-4 text-lg leading-relaxed text-muted-foreground">
          {"Pour garantir un suivi efficace, j'accepte seulement 10 eleves maximum."}
        </p>

        <div className="mt-10 mx-auto max-w-md">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-foreground">{"Places occupees"}</span>
            ...
            {Array.from({ length: totalSlots }).map((_, i) => (
              <div
                ...
                className={`h-2 w-2 rounded-full ${
                  i < takenSlots ? "bg-primary" : "bg-muted"
                }`}
              />
            ))}
          </div>
